(function (global, factory) {
  ...
  function focusToNextError() {
    ...
    var sectionCollapse =
      parent && parent.hasClassName("form-section-closed")
        ? parent.querySelector(
            'li[data-type="control_collapse"] .form-collapse-table'
          )
        : null;
    ...
    if (sectionCollapse) {
      ...
      var collapseInterval = setInterval(function () {
        if (
          !parent.hasClassName("form-section-closed") ||
          document.activeElement === erroredField
        ) {
  ...
  function close() {
    ...
    var bottomInterval = setInterval(function () {
      if (
        !state.scrollToBottomOnClose ||
        window.innerHeight + window.scrollY >= document.body.offsetHeight
      ) {
        clearInterval(bottomInterval);
        var errCont = state.section.querySelector(
  ...
  return {
    ...
    update: function update(section, render) {
      ...
      var invalidFields = state.section.querySelectorAll(
      ...
      if (state.errors.length > 0) {
        nav.querySelector(".error-navigation-next-button").style.display =
          "block";
        nav.querySelector(".error-navigation-done-button").style.display =
          "none";
